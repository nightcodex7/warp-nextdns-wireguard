...
class WarpNextDNSManager:
    ...
    def ensure_elevation(self) -> bool:
        ...
        if not self.auto_elevate_if_needed():
            logger.error(
                "Elevated privileges required. Please run with sudo/admin rights."
            )
            return False
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                "WARP Service": {
                    "status": "Running"
                    if warp_status.get("running", False)
                    else "Stopped",
                    "details": warp_status.get("details", "Service status unknown"),
                },
                "NextDNS Service": {
                    "status": "Running"
                    if nextdns_status.get("running", False)
                    else "Stopped",
                ...
                "DNS Servers": {
                    ...
                    "details": ", ".join(dns_servers)
                    if dns_servers
                    else "No DNS servers found",
                },
            }
