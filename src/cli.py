...
@click.group()
@click.option(
    "--auto", is_flag=True, help="Run in automatic mode (auto-accept prompts)"
)
@click.pass_context
def cli(ctx, auto):
...
def setup(ctx):
    ...
    console.print(
        Panel(header_text, style="panel", border_style="border", padding=(1, 2))
    )
    ...
    try:
        with Progress(
            ...
            task = progress.add_task(
                "Starting WARP and NextDNS services...", total=None
            )
...
def stop(ctx):
    ...
    try:
        with Progress(
            ...
            task = progress.add_task(
                "Stopping WARP and NextDNS services...", total=None
            )
...
def status(ctx):
    ...
    try:
        console.print(
            Panel("📊 Connection Status", style="panel", border_style="border")
        )
        ...
        for component, info in status_info.items():
            if isinstance(info, dict):
                ...
                table.add_row(
                    component, f"[{status_style}]{status}[/{status_style}]", details
                )
            else:
...
def monitor(ctx, refresh):
    ...
    console.print(
        f"[info]Refreshing every {refresh} seconds. Press Ctrl+C to stop.[/info]\n"
    )
    ...
    try:
        with Live(
            generate_status_table(), refresh_per_second=1, console=console
        ) as live:
            while True:
                time.sleep(refresh)
...
def interactive(ctx):
    ...
    def show_main_menu():
        while True:
            ...
            choice = Prompt.ask(
                "\n[primary]Select an option[/primary]",
    ...
    def show_status():
        ...
        table = Table(
            title="Current Status", title_style="title", border_style="border"
        )
...
def logs(ctx):
    ...
    try:
        ...
        console.print(
            "[info]Log viewing functionality will be implemented here.[/info]"
        )
    except Exception as e:
        console.print(f"[danger]Error: {str(e)}[/danger]")
        sys.exit(1)
...
def version():
    ...
    console.print(
        Panel(version_text, style="panel", border_style="border", padding=(1, 2))
    )
